...
class ResPartner(models.Model):
    ...
    supplier_remark = fields.Char(string=u'供应商审核备注')
    supplier_credit_limit = fields.Float(string=u'供应商信用额度', default=0.0)
    supplier_credit_limit_available = fields.Float(string=u'供应商可用额度', default=0.0)
    ...
    @api.multi
    def set_supplier_credit_available(self, credit_limit):
        """
        设置供应商信用额度
        :param credit_limit:
        :return:
        """
        if credit_limit > 0 :
            self.supplier_credit_limit = credit_limit
            return True
        else:
            raise Warning(_(u'信用额度：%s 不能小于 0' % credit_limit))
